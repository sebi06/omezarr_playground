# -*- coding: utf-8 -*-
"""
czi_omezarr_utils.display
==========================
Image display helpers:
  - get_fieldimage  — extract a scene from a 6D array as a multi-scale NgffImage
  - get_display     — read per-channel display-range settings from CZI metadata
...
def get_fieldimage(
    ...
    Args:
        array6d: 6-dimensional array with dimensions [scene, t, c, z, y, x].
        scene_index: Index of the scene to extract from the 6D array.
        metadata: Metadata object containing scale information and filename.

    Returns:
        nz.Multiscales: Multi-scale representation with scale factors [2, 2, 2].

    Notes:
        Gaussian downsampling (DASK_IMAGE_GAUSSIAN method) is used for pyramid generation.
...
def get_display(metadata: CziMetadata, channel_index: int) -> tuple[float, float, float]:
    ...
    Args:
        metadata: CziMetadata object with channel display settings.
        channel_index: Zero-based channel index.

    Returns:
        tuple[float, float, float]: (lower, higher, maxvalue) where lower/higher are
            the display window bounds and maxvalue is the absolute channel maximum.

    Note:
        Falls back to (0, maxvalue, maxvalue) when metadata is missing or corrupted.
    """
...
def create_channel_list(metadata: CziMetadata) -> list:
    ...
    Args:
        metadata: CziMetadata with channelinfo and maxvalue_list populated.

    Returns:
        List of channel dicts with keys: color, label, active, window.
    """
    channels_list: list = []
